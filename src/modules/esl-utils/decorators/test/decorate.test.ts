...
describe('common @decorate decorator test', () => {
  describe('Decorated method access leads to decorated instance access', () => {
    ...
    test('Decorated with @decorator method returns wrapped method', () =>
      expect(t1.test()).toBe(v1));
    test('@decorator called ones it\'s accessed', () => expect(wrap).toBeCalledTimes(1));
    test('Decorated with @decorator method returns wrapped method every time', () =>
      expect(t1.test).toBe(fn1));
    test('@decorator does not redecorate method second time', () =>
      expect(wrap).toBeCalledTimes(1));

    const t2 = new Test();
    test('@decorator doesn\'t have effect on instance creation', () =>
      expect(wrap).toBeCalledTimes(1));
    test('Second instance creates its own wrapped method', () =>
      expect(t2.test()).toBe(v2));
    test('Decorated with @decorator method returns wrapped method every time', () =>
      expect(t2.test).toBe(fn2));

    const t3 = new Test();
    test('Original function passed as a first argument for wrapper', () =>
      expect(t3.test()).toBe(original));
  ...
  describe('Decorated method inheritance works correct', () => {
    ...
    test('Arguments passed correctly', () => expect(originalFn).lastCalledWith(arg1));
    test('Second call still works correct', () =>
      expect(instance.test(arg1)).toBe(original));
    test('Prototype method call run function without decoration', () =>
      expect(Parent.prototype.test()).toBe(original));
    test('Decoration does not happens on super access', () =>
      expect(wrap).toBeCalledTimes(0));

    const instance2 = new Child();
    test('Call of super keyword returns original method', () =>
