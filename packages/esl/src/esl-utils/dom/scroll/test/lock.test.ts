import {isScrollLocked, lockScroll, unlockScroll} from '../lock';
...
describe('Function lockScroll', () => {
  ...
  describe('Lock scroll automatically loads related styles', () => {
    test('styles should be injected upon lock', () => {
      lockScroll();
      const styleElement = document.getElementById('esl-scroll-lock-styles');
      expect(styleElement).not.toBeNull();
      expect(styleElement?.tagName).toBe('STYLE');
    });

    test('styles should be injected only once', () => {
      const initialStyleElement = document.getElementById('esl-scroll-lock-styles');
      lockScroll();
      const subsequentStyleElement = document.getElementById('esl-scroll-lock-styles');
      expect(subsequentStyleElement).toBe(initialStyleElement);
    });
  });
});
